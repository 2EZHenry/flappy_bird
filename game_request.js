class GameRequest {
	...
	update_user_data(res, on_complete) {
		let key;
		for (key in res)
			if (key in game_data["user_data"]) game_data["user_data"][key] = res[key];
	...
	save_user_data() {
		if (is_localhost) {
			localStorage.setItem(
				loading_vars["game_id"] + "_" + `user_data`,
				JSON.stringify(game_data["user_data"])
			);
	...
	local_request(obj, on_complete) {
		...
		if ("update_current_resourse" in obj)
			this.update_current_resourse_local(obj, on_complete);
		if ("buy_level_continue" in obj)
			this.buy_level_continue_local(obj, on_complete);
	...
	collect_money_local(obj, on_complete) {
		...
		if (game_data["user_data"]["money"] < 0)
	...
	buy_item_local(obj, on_complete) {
		...
		if (type === "money") {
			...
			if (
				item &&
				(!(item["id"] in ad_watched) || item["price"] >= ad_watched[item["id"]])
			) {
				success = true;
		...
		on_complete({
			...
			money: money,
			id: id,
			money_resourses: money_resourses,
			current_resourse: current_resourse,
			ad_watched: ad_watched,
			ad_resourses: ad_resourses,
		});
	}
	...
	level_failed_local(obj, on_complete) {
		...
		if (score > game_data["user_data"]["global_score_mode"][mode]) {
			game_data["user_data"]["old_global_score_mode"][mode] =
				game_data["user_data"]["global_score_mode"][mode];
	...
	get_game_info_local(obj, on_complete) {
		if (is_localhost) {
			let saved_user_data = localStorage.getItem(
				loading_vars["game_id"] + "_" + `user_data`
			);
			if (
				saved_user_data &&
				!("clear_storage" in game_data && game_data["clear_storage"])
			) {
			...
			let saved_user_data = YAPI.getItem(
				loading_vars["game_id"] + "_" + `user_data`
			);
			if (
				saved_user_data &&
		...
		if (!("start_day_id" in game_data["user_data"]))
			game_data["user_data"]["start_day_id"] = curent_day_id;

		if (new_user && game_data["new_lang"])
			game_data["user_data"]["lang"] = game_data["new_lang"];

		if (!("session_id" in game_data["user_data"]))
			game_data["user_data"]["session_id"] = 0;
		game_data["user_data"]["session_id"]++;
		let session_id =
			loading_vars["game_id"] +
			"_" +
			loading_vars["net_id"] +
			"_" +
			user_id +
