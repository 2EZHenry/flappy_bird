let LevelFailed = new Phaser.Class({
	...
	initialize: function LevelFailed() {
	...
	init(params) {
		...
		this.text1 = this.scene.add.bitmapText(
			0,
			-300,
			"font",
			res["text"],
		...
		this.text3 = this.scene.add.bitmapText(
			0,
			10,
			"font",
			res["text"],
		...
		this.text4 = this.scene.add.bitmapText(
			23,
			5,
			"font",
			res["text"],
		...
		this.button_close = new CustomButton(
			game_data["scene"],
			0,
			330,
			this.handler_cancel,
		...
		let ad_ico = new Phaser.GameObjects.Image(
			this.scene,
			this.button_play.x - 72,
			0,
			"common1",
			"rewarded_ad"
		);
		...
		game_data["utils"].show_rewarded_ad((res) => {
			if (res["success"]) {
				...
				game_data["utils"].show_tip({
					pt: pt,
					text: "Ads are not available now,\ntry again later",
					forced: true,
					values: [],
				});
			}
