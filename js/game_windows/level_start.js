let LevelStart = new Phaser.Class({
	...
	initialize: function LevelStart() {
	...
	init(params) {
		game_data["graphics_manager"].get_window(
			"info",
			this.handler_close,
			[{ handler: this.handler_replay, type: "big" }],
			this,
			null,
			true
		);
	...
	create_assets() {
		let add_y = 0;
		this.btn_easy = new CustomButton(
			this.scene,
			0,
			-140 + add_y,
			() => {
		...
		temp = new Phaser.GameObjects.Text(this.scene, 0, -3, res["text"], {
		...
		this.btn_normal = new CustomButton(
			this.scene,
			0,
			-50 + add_y,
			() => {
		...
		this.btn_hard = new CustomButton(
			this.scene,
			0,
			40 + add_y,
			() => {
		...
		this.btn_crazy = new CustomButton(
			this.scene,
			0,
			130 + add_y,
			() => {
