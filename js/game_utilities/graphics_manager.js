class GraphicsManager {
	...
	get_window(
		...
		if (close_handler) {
			let button_close = new CustomButton(
				game_data["scene"],
				close_pos.x,
				close_pos.y,
				close_handler,
		...
		if (title_obj) {
			...
			context.title = new Phaser.GameObjects.Text(
				this.scene,
				title_pos.x,
				title_pos.y,
				"",
		...
		for (i = 0; i < btn_info.length; i++) {
			...
			button.setInteractive(); // Enable input events

			button.on("pointerover", () => button.setScale(btn_scale * 1.1));
			button.on("pointerout", () => button.setScale(btn_scale));
